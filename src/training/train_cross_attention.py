"""
Phase 4: Cross-Attention Fusion Baseline with Group-Aware 5-Fold CV.

Trains a model that uses cross-attention between EfficientNet-B0 image spatial
features and DistilBERT text token features to predict severity.
Image features act as Queries, Text features act as Keys/Values.

Usage:
    conda run -n genai python src/training/train_cross_attention.py
"""
...
D_K            = 512
NUM_HEADS      = 8
MODALITY_DROP  = 0.1    # Randomly zero out one modality per batch (10% each)
...
def train_fold(
    ...
    model = CrossAttentionFusionClassifier(
        ...
        d_k=D_K,
        num_heads=NUM_HEADS,
        modality_dropout_p=MODALITY_DROP,
    ...
    # Separate parameter groups — different LRs for encoders vs classification head & cross-attention
    encoder_params    = list(model.image_encoder.parameters()) + \
                        list(model.text_encoder.parameters())
    classifier_params = list(model.query_proj.parameters()) + \
    ...
    model_path = RESULTS_DIR / f"cross_attention_fusion_fold{fold_idx+1}.pt"
...
def evaluate_held_out_test(
    ...
    model = CrossAttentionFusionClassifier(
        ...
        d_k=D_K,
        num_heads=NUM_HEADS,
        modality_dropout_p=0.0,  # No dropout during test evaluation
    ).to(DEVICE)
    ...
    print(f"\n  {'='*45}")
    print(f"  COMPARISON VS UNIMODAL BASELINES & CONCAT")
    print(f"  {'='*45}")
    ...
    print(f"  Cross-Attention:      Acc={metrics['accuracy']*100:.1f}%  F1={metrics['f1_macro']*100:.1f}%")

    baseline_f1 = 0.732  # best baseline (Concat)
    if metrics["f1_macro"] > baseline_f1:
        gain = (metrics["f1_macro"] - baseline_f1) * 100
        print(f"\n  [PASS] Cross-attention beats Concat baseline by +{gain:.1f}% F1")
        print(f"  -> Cross-modal interaction provides real benefit.")
    else:
        gap = (baseline_f1 - metrics["f1_macro"]) * 100
        print(f"\n  [NOTE] Cross-attention is {gap:.1f}% F1 below Concat baseline.")
        print(f"  -> May need hyperparameter tuning or more data.")
...
def main():
    print("=" * 60)
    print("  Phase 4: Cross-Attention Fusion (Image + Text)")
    ...
    print(f"  d_k:          {D_K}        |  Num Heads:     {NUM_HEADS}")
    print(f"  Modality Dropout: {MODALITY_DROP}")
    print(f"  L2 Normalization: ON")
    print()
    ...
    # Save all results
    all_results = {
        "model":   "Cross-Attention Fusion (EfficientNet-B0 + DistilBERT)",
        ...
        "baselines": {
            ...
            "concat_test_f1":      0.732,
        },
    }

    results_file = RESULTS_DIR / "phase4_cross_attention_v2_results.json"
